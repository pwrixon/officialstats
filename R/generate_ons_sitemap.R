...
#'
#' This function generates a sitemap for the ONS taxonomy API.
#'
#' @param base_url The base URL of the ONS API.
#'
#' @return A sitemap for the ONS taxonomy API.
#' @export
#'
#' @examples
#' get_ons_taxonomy("https://api.ons.gov.uk")
...
#' Convert ONS taxonomy data into a flat structure with additional columns
#'
#' @param ons_taxonomy_all A tibble containing the ONS taxonomy data
#' @return A tibble with the ONS taxonomy data in a flat structure with additional columns
#' @export
convert_ons_taxonomy <- function(ons_taxonomy_all) {
  ons_taxonomy_all <- ons_taxonomy_all |>
    ...
    dplyr::add_row(title = "Home", type = "home_page",
                   parent.uri = NA, uri = "/",
                   .before = 1) |>
